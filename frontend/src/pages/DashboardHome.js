      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
        {cards.map((card) => (
          <Link
            key={card.label}
            to={card.to}
            className="bg-grafite-900 border border-grafite-800 rounded-xl p-5 md:p-6 hover:border-laranja-600/30 transition-all active:scale-[0.98]"
          >
      ...
      <div className="bg-grafite-900 border border-grafite-800 rounded-xl p-4 md:p-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
          <h3 className="text-white font-semibold text-sm md:text-base flex items-center gap-2">
            <BarChart3 size={18} className="text-laranja-400" />
            Gráficos
          </h3>
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                className="bg-grafite-800 border border-grafite-700 rounded-lg px-2 py-1.5 text-xs text-white focus:outline-none focus:ring-1 focus:ring-laranja-600"
              />
            </div>
            <div className="flex items-center gap-2">
              <Calendar size={14} className="text-gray-400 shrink-0" />
        ...
        {chartsLoading ? (
          <div className="text-center text-gray-500 text-sm py-8">Carregando gráficos...</div>
        ) : !charts ? (
          <div className="text-center text-gray-500 text-sm py-8">Erro ao carregar gráficos</div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div>
              <div className="flex items-center gap-2 mb-3">
                <Activity size={16} className="text-green-400" />
                <h4 className="text-white text-sm font-medium">Faturamento Diário</h4>
              </div>
              {charts.faturamento_diario.length === 0 ? (
                <p className="text-gray-500 text-xs text-center py-6">Nenhum faturamento no período</p>
              ) : (
                <div className="flex items-end gap-1 h-32 md:h-40">
                  {charts.faturamento_diario.map((item, i) => (
                    <div key={i} className="flex-1 flex flex-col items-center justify-end h-full min-w-0">
                      <span className="text-[10px] text-gray-400 mb-1 truncate w-full text-center">{formatCurrency(item.total)}</span>
                      <div
                      ...
                      />
                      <span className="text-[10px] text-gray-500 mt-1">{formatDateBR(item.dia)}</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="text-white text-sm font-medium">OS por Status</h4>
              </div>
              {charts.os_por_status.length === 0 ? (
                <p className="text-gray-500 text-xs text-center py-6">Nenhuma OS no período</p>
              ) : (
                    ...
                    return (
                      <div key={i}>
                        <div className="flex justify-between text-xs mb-1">
                          <span className="text-gray-300">{STATUS_LABELS[item.status] || item.status}</span>
                          <span className="text-white font-medium">{item.total}</span>
                        </div>
                        <div className="w-full bg-grafite-800 rounded-full h-3">
                          <div
