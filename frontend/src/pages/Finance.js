    ...
    <div className="space-y-4 pb-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={exportCSV} className="flex items-center gap-2 bg-grafite-700 hover:bg-grafite-600 text-white px-4 py-2.5 md:py-2 rounded-xl md:rounded-lg text-sm font-medium">
            <Download size={18} />
      ...
      {showForm && (
        <form onSubmit={save} className="bg-grafite-900 border border-grafite-800 rounded-xl p-4 md:p-5 space-y-3">
          <div className="flex gap-2">
            <button type="button" onClick={() => setForm({ ...form, tipo: 'entrada' })} className={`flex-1 py-2.5 rounded-lg text-sm font-medium transition-all ${form.tipo === 'entrada' ? 'bg-green-600 text-white' : 'bg-grafite-800 text-gray-400'}`}>
              <ArrowUpRight size={16} className="inline mr-1" /> Entrada
            </button>
            <button type="button" onClick={() => setForm({ ...form, tipo: 'saida' })} className={`flex-1 py-2.5 rounded-lg text-sm font-medium transition-all ${form.tipo === 'saida' ? 'bg-red-600 text-white' : 'bg-grafite-800 text-gray-400'}`}>
              <ArrowDownRight size={16} className="inline mr-1" /> Saída
            </button>
          </div>
          <input placeholder="Descrição" value={form.descricao} onChange={(e) => setForm({ ...form, descricao: e.target.value })} className="w-full bg-grafite-800 border border-grafite-700 rounded-lg px-4 py-3 text-white text-sm focus:outline-none focus:border-laranja-500" required />
          <input placeholder="Valor (R$)" type="number" step="0.01" min="0.01" value={form.valor} onChange={(e) => setForm({ ...form, valor: e.target.value })} className="w-full bg-grafite-800 border border-grafite-700 rounded-lg px-4 py-3 text-white text-sm focus:outline-none focus:border-laranja-500" required />
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="flex items-center gap-3 mb-4">
            <div className="flex items-center gap-2">
              <label className="text-xs text-grafite-400">De:</label>
              <input
                type="date"
                value={dataInicio}
                onChange={e => { setDataInicio(e.target.value); setPage(1); }}
                className="bg-grafite-700 border border-grafite-600 rounded-lg px-3 py-1.5 text-sm text-white"
              />
            </div>
            <div className="flex items-center gap-2">
              <label className="text-xs text-grafite-400">Até:</label>
              <input
                type="date"
                value={dataFim}
            ...
            {(dataInicio || dataFim) && (
              <button
                ...
                className="text-xs text-laranja-400 hover:text-laranja-300"
              >
                Limpar filtro
              </button>
            )}
